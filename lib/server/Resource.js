...
var Resource = function(resourceInfo) {
    ...
    this._createRepository(resourceInfo.repository, resourceInfo.repositoryOptions);
...
Resource.prototype.addRoutes = function(server) {
    ...
    server.put(resourceUriWithIdField, function(req, res) {
        if(server.debug)
...
Resource.prototype._createRepository = function(Repository, repositoryOptions) {
    var dataInfo = {
        idField: this.idField,
        name : this.name
    };
    this.repository = new Repository(dataInfo, repositoryOptions);
};
